...
def get_sort_key(discipline_title):
    ...
    stroke_idx = stroke_order.index(stroke) if stroke in stroke_order else len(stroke_order)
    distance_idx = distance_order.index(distance) if distance in distance_order else len(distance_order)
...
def best_records_query(session : Session, gender : Literal["M" ,"Z"], course_length : int):
    ...
    subq = (
        session.query(
            PB.discipline_id,
            func.min(PB.time).label("min_time")
        )
        .join(SW, PB.swimmer_id == SW.id)
        .join(CO, PB.course_id == CO.id)
        .filter(
            SW.gender == gender,
            CO.length == course_length
        )
    ...
    q = (
        ...
        .join(SW, PB.swimmer_id == SW.id)
        .join(subq, and_(
            subq.c.discipline_id == PB.discipline_id,
            subq.c.min_time == PB.time
        ))
        .filter(
            SW.gender == gender,
            CO.length == course_length
